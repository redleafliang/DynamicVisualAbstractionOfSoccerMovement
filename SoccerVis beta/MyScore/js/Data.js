...
function GetInfo() {
    ...
    xmlhttp.onreadystatechange = function () {
        if (xmlhttp.readyState == 4 && xmlhttp.status == 200) {
           ...
           xmlhttp.open('GET', "./backend/GetInfo.php", false);
           xmlhttp.setRequestHeader("Content-type", "application/x-www-form-urlencoded");
           xmlhttp.send();

    // xmlhttp.open("POST", "./backend/GetInfo.php", true);
    // xmlhttp.setRequestHeader("Content-type", "application/x-www-form-urlencoded");
    // xmlhttp.send();
}

function SetInfo(InfoValue, InfoLocation) {
    var xmlhttp;
    ...
    xmlhttp.open('GET', "./backend/SetInfo.php?InfoValue="+InfoValue+"&InfoLocation="+InfoLocation, true);
    xmlhttp.setRequestHeader("Content-type", "application/x-www-form-urlencoded");
    xmlhttp.send();
}
...
Data.prototype.createSettingsButtons = function(){
    ...
    var DefaultValue = parseInt(json["div3"].mysize);
    this.globalFlowSize = DefaultValue;
    ...
    div6.append("label")
        .html("layer Controller: <br>");


    var DefaultValueArray = [parseInt(json["div6"].mynormal),parseInt(json["div6"].CatmullRomChain),parseInt(json["div6"].SIA1),
        parseInt(json["div6"].SIA2),parseInt(json["div6"].SIA3),parseInt(json["div6"].SIA4),parseInt(json["div6"].ComputeBezier1),
        parseInt(json["div6"].ComputeBezier2),parseInt(json["div6"].ComputeBezier3),parseInt(json["div6"].ComputeBezier4),parseInt(json["div6"].ComputeBezier5),
        parseInt(json["div6"].ComputeBezier6),parseInt(json["div6"].kMeansOrkMedoids1),parseInt(json["div6"].kMeansOrkMedoids2),parseInt(json["div6"].kMeansOrkMedoids3),
        parseInt(json["div6"].kMeansOrkMedoids4),parseInt(json["div6"].kMeansOrkMedoids5),parseInt(json["div6"].kMeansOrkMedoids6),parseInt(json["div6"].kMeansOrkMedoids7),
        parseInt(json["div6"].kMeansOrkMedoids8),parseInt(json["div6"].kMeansOrkMedoids9),parseInt(json["div6"].kMeansOrkMedoids10)
        ];
    DefaultValue = DefaultValueArray.indexOf(Math.max.apply(Math, DefaultValueArray)) * 10;
    div6.append("div").attr("id","layerController").style("width","100px");
    $("#layerController").slider({
        range: "min",
        max: 210,
        min: 0,
        value: DefaultValue,
        step: 10,
        slide: function(event,ui){hydraw(ui.value);SetInfo(ui.value,"layerController");}
    });

    div6.append("label")
        .html("<br> cluster algorithm  <br>");

    var clusterAlgorithm = ["k-means","k-medoids"];

    var clusterAlgorithmSelect = div6.append("select")
        .attr("class","clusterAlgorithmSelect")
        .on("change", function(){$this.selectClusterAlgorithmMode()});

    clusterAlgorithmSelect
        .selectAll(".clusterAlgorithmOption")
        .data(clusterAlgorithm)
        .enter()
        .append("option")
        .attr("class","clusterAlgorithmOption")
        .attr("name", "clusterAlgorithmOption")
        .attr("value", function(d){return d})
        .html(function(d){return d});

    div6.append("label")
        .html("<br> search player<br>");
    ...
    div6.append("button")
        ...
        .on("click", function(){
            ...
            if (isNaN(InputValue)) {
                ...
                SelectPlayerByName(InputValue);
            }
            else {
                //console.log("a number");
                SelectPlayerByNumber(InputValue);
            }
...
Data.prototype.selectAnimateClusteringDuration = function(){
    ...
    //SetInfo($('select option[name=animateClusteringDuration]:selected').val(),"AnimateClusteringDuration");
};
Data.prototype.selectAnimateSequenceDuration = function(){
    ...
    SetInfo($('select option[name=animateSequenceDuration]:selected').val(),"AnimateSequenceDuration");
};
...
Data.prototype.selectPassClusterStyle = function(){
    ...
    SetInfo($('select option[name=passClusterStyleOption]:selected').val(),"PassClusterStyle");
};
...
Data.prototype.selectGlobalFlowInterpolate = function(){
    ...
    SetInfo($('select option[name=globalFlowInterpolateOption]:selected').val(),"GlobalFlowInterpolate");
    if(this.selected_sequence != undefined){
...
Data.prototype.changeGlobalFlowSize = function(event,ui){
    ...
    SetInfo(ui.value,"GlobalFlowSize");
    if(this.selected_sequence != undefined){
...
Data.prototype.selectSequenceVisMode = function(){
    ...
    SetInfo($('select option[name=sequenceVisModeOption]:selected').val(),"SequenceVisMode");
    SetInfo($('select option[name=sequenceVisDurationOption]:selected').val(),"SequenceVisDuration");
    SetInfo($('select option[name=sequenceVisDelayOption]:selected').val(),"SequenceVisDelay");
...
Data.prototype.selectClusterAlgorithmMode = function(){
    this.ClusterAlgorithmMode = $('select option[name=clusterAlgorithmOption]:selected').val();
    console.log(this.ClusterAlgorithmMode);
    this.selected_sequence.setClusterAlgorithmMode(this.ClusterAlgorithmMode);
};
...
Data.prototype.init = function(){
    ...
    var DefaultValue = Math.round((json["div1"].ClusteringDuration)/100);
    $(".clusteringDurationSelect").prop('selectedIndex', DefaultValue); //div1 clustering duration


    DefaultValue = Math.round((json["div1"].SequencecAnimation)/100);
    $(".sequenceDurationSelect").prop('selectedIndex', DefaultValue); //div1 sequence animation


    var myNodeCircle = json["div1"].nodecircle;
    var myNodeJersey = json["div1"].nodejersey;
    if(myNodeCircle >= myNodeJersey)
        {DefaultValue = 0;}
    else
        {DefaultValue = 0;}
    $(".nodesStyleSelect").prop('selectedIndex', DefaultValue);  //div1 node style

    var DefaultValueArray = [parseInt(json["div1"].nodeLink),parseInt(json["div1"].nodeLinkAll),parseInt(json["div1"].hiveplot),parseInt(json["div1"].matrix),parseInt(json["div1"].tagcloud)];
    DefaultValue = DefaultValueArray.indexOf(Math.max.apply(Math, DefaultValueArray))
    $(".passClusterStyleSelect").prop('selectedIndex', DefaultValue);  //div1 pass cluster style


    $(".globalFlowInterpolateSelect").prop('selectedIndex', DEFAULT_GLOBAL_FLOW_INTERPOLATE);  // div3 global flow

    var mytwoD = json["div4"].twoD;
    var myhorizontal = json["div4"].myhorizontal;
    if(mytwoD >= myhorizontal)
    {DefaultValue = 0;}
    else
    {DefaultValue = 1;}
    $(".sequenceLayoutSelect").prop('selectedIndex', DefaultValue); //div4 sequence layout

    //DefaultValueArray.splice(0,DefaultValueArray.length);
    DefaultValue = Math.round((json["div4"].avgDuration)/100);
    $(".sequenceLayoutDurationSelect").prop('selectedIndex', DefaultValue); //div4 sequence layout duration

    DefaultValue = Math.round((json["div4"].avgDelay)/100);
    $(".sequenceLayoutDelaySelect").prop('selectedIndex', DefaultValue);//div4 sequence layout delay

    DefaultValueArray.splice(0,DefaultValueArray.length);
    DefaultValueArray = [parseInt(json["div2"].hiveplot),parseInt(json["div2"].nodeLink),parseInt(json["div2"].nodeLinkAll),parseInt(json["div2"].tagcloud),parseInt(json["div2"].matrix)];
    DefaultValue = DefaultValueArray.indexOf(Math.max.apply(Math, DefaultValueArray))
    $(".switchVisTypeSelect").prop('selectedIndex', DefaultValue); //div2 vis type

    DefaultValue = Math.round((json["div2"].VisDuration)/100);
    $(".switchVisDurationSelect").prop('selectedIndex', DefaultValue); //div2 vis duration

    DefaultValueArray.splice(0,DefaultValueArray.length);
    DefaultValueArray = [parseInt(json["div5"].myScaled),parseInt(json["div5"].myScaledLinks),parseInt(json["div5"].mySignature),parseInt(json["div5"].myWorm),
        parseInt(json["div5"].myXProj),parseInt(json["div5"].myYProj),parseInt(json["div5"].myTimeAlign),parseInt(json["div5"].myDistanceAlign),
        parseInt(json["div5"].myDonut)];
    DefaultValue = DefaultValueArray.indexOf(Math.max.apply(Math, DefaultValueArray))
    $(".sequenceVisModeSelect").prop('selectedIndex', DefaultValue); //div5 sequence vis

    DefaultValue = Math.round((json["div5"].avgDuration)/100);
    $(".sequenceVisDurationSelect").prop('selectedIndex', DefaultValue); // div5 duration

    DefaultValue = Math.round((json["div5"].avgDelay)/100);
    $(".sequenceVisDelaySelect").prop('selectedIndex', DefaultValue); //div5 delay


    $(".sequenceVisCheckField").prop('checked', true); //div5 draw field

    $(".exportModeSelect").prop('selectedIndex', DEFAULT_EXPORT_MODE);


    if(parseInt(json["div6"].kMeans) >= parseInt(json["div6"].kMedoids))
    {DefaultValue = 0;}
    else
    {DefaultValue = 1;}
    $(".clusterAlgorithmSelect").prop('selectedIndex', DefaultValue);
